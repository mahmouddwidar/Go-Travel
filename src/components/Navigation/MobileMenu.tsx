...
export default function MobileMenu() {
  ...
  return (
    <motion.div
      ...
      className="fixed top-0 right-0 bottom-0 left-0 justify-end bg-white/30 pl-30"
    >
      <motion.nav
        animate={menuOpened ? "visible" : "hidden"}
        variants={{
      ...
      >
        ...
        >
          {navigationLinks.map((link) => (
            <li
              key={link.id}
              className={`group relative flex flex-col font-medium transition-all duration-300 ease-in-out ${activeLinkId === link.id ? "gap-y-6" : "gap-y-0"}`}
              onClick={() =>
            ...
            >
              <div className="flex cursor-pointer items-center gap-x-1.75">
                <a
                  href="#"
                  className={`text-grey-600 ${activeLinkId === link.id ? "text-grey-1000" : ""}`}
                >
                  {link.text}
                </a>
                {link.dropdown && (
                  ...
                  >
                    <CaretUp
                      className={`fill-grey-600 size-3.5 rotate-180 transition-all duration-200 ${link.id === activeLinkId ? "fill-grey-1000" : ""}`}
                    />
                  </span>
                )}
              </div>
